...
export const dueThisWeek = (bills: BillType[]) => {
  ...
  for (const bill of bills) {
    ...
    if (
      ...
      const target = (occ.day - 1 + 7) % 7;
      const hit = days.find((d) => d.getDay() === (target + 1) % 7);

      if (hit) {
        const wk = isoWeekNumber(hit);
        if (occ.week === 1 || wk % occ.week === 0) matches.push(bill);
      }
    }
  }
  ...
  return { count, total, bills: matches };
};
